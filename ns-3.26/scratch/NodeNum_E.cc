...
//
/**该脚本用于测试业务类型C的饱和吞吐量的情况
 *  业务类型C：包大小 100B，发包速率 100 个/s
 *  测试内容：
 *      收包、吞吐量、时延
 */
...
int main(int argc, char *argv[]) {
    ...
    double simulation_time = ((double)ttnt / 2) * 57;
    ...
    if (workflow[8]) {
         ...
         */
        {
            uint32_t packetSize = 420; // odd number:1000, even number:500
            uint32_t maxPacketCount = 10000000;
            uint32_t packetFrequency = 20;
            Time interPacketInterval = Seconds((1 / (double) packetFrequency) /*1/((double)20)*/);
