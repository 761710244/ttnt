...
int main(int argc, char *argv[]) {
    ...
    mobility.SetPositionAllocator("ns3::GridPositionAllocator",//按照设置好的行列参数把节点等间距放置在一个二维笛卡尔坐标系中
                                  ...
                                  "DeltaX", DoubleValue(0.01), // X轴节点间距：0.01m
                                  "DeltaY", DoubleValue(0.01), // y轴节点间距：0.01m
                                  "GridWidth", UintegerValue(5),  // 每行最大节点数
    ...
    uint32_t simulation_time = (ttnt / 2) * 57;
    ...
    if (workflow[8]) {
         ...
         */
        {
            uint32_t packetSize = 200; // odd number:1000, even number:500
            uint32_t maxPacketCount = 10000000;
            uint32_t packetFrequency = 20;
            Time interPacketInterval = Seconds((1 / (double) packetFrequency) /*1/((double)20)*/);
    ...
    if (workflow[11]) {
         ...
         */
        {
            uint32_t packetSize = 300; // odd number:1000, even number:500
            uint32_t maxPacketCount = 10000000;
            uint32_t packetFrequency = 20;
            Time interPacketInterval = Seconds((1 / (double) packetFrequency) /*1/((double)20)*/);
    ...
    Simulator::Stop(Seconds((double)simulation_time));
