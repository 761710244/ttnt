...
//    adhocMobility.Install (TTNTNode);
    ...
    uint16_t record_time = 50;
    for (uint32_t i = 1; i <= (2 / 2); i++) {
        workflow[i] = 1;

        routing_start[i] = (i - 1) * 57;
        routing_end[i] = routing_start[i] + 5;

        data_start[i] = routing_start[i] + 6;
        data_end[i] = data_start[i] + record_time;
    }
    double simulation_time = (2 / 2) * 57;
    ...
    if (workflow[2]) {
         ...
         */
        {
            ...
            UdpServerHelper echoServerv(112);
            ApplicationContainer serv1erAppsv = echoServerv.Install(TTNTNode.Get(4)); // --
            serv1erAppsv.Start(Seconds(routing_start[2]));
            serv1erAppsv.Stop(Seconds(routing_end[2]));

            UdpClientHelper echoClient(ttntInterface.GetAddress(4), 112);
            echoClient.SetAttribute("MaxPackets", UintegerValue(maxPacketCountv));
            echoClient.SetAttribute("Interval", TimeValue(interPacketIntervalv));
            echoClient.SetAttribute("PacketSize", UintegerValue(packetSizev));
            ...
            ApplicationContainer clientAppsv = echoClient.Install(
                    TTNTNode.Get(1));
            clientAppsv.Start(Seconds(routing_start[2]));
            clientAppsv.Stop(Seconds(routing_end[2]));
        }
         ...
         */
        {
            ...
            UdpServerHelper echoServerv(22);
            ApplicationContainer serverAppsv = echoServerv.Install(TTNTNode.Get(4));
            serverAppsv.Start(Seconds(data_start[2]));
            serverAppsv.Stop(Seconds(data_end[2]));

            UdpClientHelper echoClient(ttntInterface.GetAddress(4), 22);
            echoClient.SetAttribute("MaxPackets", UintegerValue(maxPacketCount));
            echoClient.SetAttribute("Interval", TimeValue(interPacketInterval));
            echoClient.SetAttribute("PacketSize", UintegerValue(packetSize));

            ApplicationContainer clientApps = echoClient.Install(TTNTNode.Get(1));
            clientApps.Start(Seconds(data_start[2]));
            clientApps.Stop(Seconds(data_end[2]));
        }
    }
