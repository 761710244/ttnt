...
namespace ns3 {
    ...
    namespace dsr {
        ...
        bool DsrOptions::ContainAddressAfter(Ipv4Address ipv4Address, Ipv4Address destAddress,
...
//
//            uint16_t j = ipv4->GetNInterfaces();
//            for(uint16_t k = 1; k<j; k++)
//            {
//                if (ipv4->GetAddress (k, 0).GetLocal () == address)
//                  {
//              	  std::cout<<"ipv4->GetAddress (x, 0).GetLocal () "<<ipv4->GetAddress (k, 0).GetLocal ()<<"\n";
...
//    }
        ...
        uint8_t
        DsrOptionPad1::Process(Ptr <Packet> packet, Ptr <Packet> dsrP, Ipv4Address ipv4Address, Ipv4Address source,
        ...
        uint8_t
        DsrOptionPadn::Process(Ptr <Packet> packet, Ptr <Packet> dsrP, Ipv4Address ipv4Address, Ipv4Address source,
                               Ipv4Header const &ipv4Header, uint8_t protocol, bool &isPromisc,
                               Ipv4Address promiscSource) {
            NS_LOG_FUNCTION(
                    this << packet << dsrP << ipv4Address << source << ipv4Header << (uint32_t) protocol << isPromisc);
        ...
        uint8_t
        DsrOptionRreq::Process(Ptr <Packet> packet, Ptr <Packet> dsrP, Ipv4Address ipv4Address, Ipv4Address source,
                               Ipv4Header const &ipv4Header, uint8_t protocol, bool &isPromisc,
                               Ipv4Address promiscSource) {
            NS_LOG_FUNCTION(
                    this << packet << dsrP << ipv4Address << source << ipv4Header << (uint32_t) protocol << isPromisc);
             ...
             */
            std::map < Ipv4Address, Ptr < Ipv4L3Protocol >> ::iterator
            ptr;
            ptr = nodeInterface.begin();
            ...
            if (ipv4Address == Ipv4Address("198.2.1.4"))
                cout << "Original sourcre " << source << " Now: " << Simulator::Now() << endl;
            /*
            ...
            if (Kind == "None") {
                ...
                if (numberAddress >= NORMAL_WINDOW) {
                    m_dropTrace(packet);
                    return 0;
                }
            } else if (Kind == "ReRouting") {
                if (numberAddress >= RE_ROUTING_WINDOW) {
                    m_dropTrace(packet);
                    return 0;
                }
...
// for(auto i=nodeList.begin();i!=nodeList.end();i++) cout<<"lalala"<<*i<<endl;
            set <Ipv4Address> s1 = {"198.3.1.26", "198.3.1.27", "198.3.1.28", "198.2.1.1"};
            set <Ipv4Address> s2 = {"198.3.1.26", "198.3.1.27", "198.3.1.28", "198.3.1.29", "198.3.1.30", "198.3.1.31",
                                    "198.2.1.1"};
            //黑洞节点收到RREQ时.如果SRC是黑洞节点并且路径中也有黑洞节点那么DROP(防止多个黑洞节点联合影响)
            if (0) {
                maliciousIpAddr = s2;
                if (/*sourceAddress == Ipv4Address("198.2.1.1") && */nodeList.size() > 3) {
                    for (auto iNode = nodeList.begin() + 1; iNode != nodeList.end();
                         iNode++) {
                        auto mIp = find(maliciousIpAddr.begin(), maliciousIpAddr.end(), *iNode);
                        if (mIp != maliciousIpAddr.end()) {
                            m_dropTrace(packet); // call drop trace
                            return 0;
                        }
                    }
            ...
            // MAC DOS攻击时恶意节点并不参与正常RREQ过程
            if (0) {
                vector <Ipv4Address> maliciousIpv4 = {"198.1.1.26", "198.1.1.27", "198.1.1.28", "198.1.1.29",
                                                      "198.1.1.30", "198.1.1.31", "198.1.1.32"};
                auto srcIter = find(maliciousIpv4.begin(), maliciousIpv4.end(), sourceAddress);
                if (srcIter == maliciousIpv4.end()) {
                    for (auto iter = nodeList.begin(); iter != nodeList.end(); iter++) {
                        auto findM = find(maliciousIpv4.begin(), maliciousIpv4.end(), *iter);
                        if (findM != maliciousIpv4.end()) {
                            m_dropTrace(packet); // call drop trace
                            return 0;
                        }
                    }
            ...
            if (dsrP->GetMaliciousType() == 2) {
                std::cout << "******** Malicious = " << ipv4Address << std::endl;
                maliciousIpAddr = s2;
            }
            /*
            ...
            dsrRoutingHeader.SetDestId(
                    ipv4Address.GetSubnetDirectedBroadcast(Ipv4Mask("255.255.255.0")));//DESTID根据当前的interface改变DESTID
            ...
            //**************************************************************  BlackHole One, Malicious Send RREP ODIE
            if (((dsrP->GetMaliciousType() == 1) || (dsrP->GetMaliciousType() == 2)) &&
                maliciousIpAddr.find(source) == maliciousIpAddr.end()) {
                //todo 修改虚假最短RREP
                Ipv4Address maliciousIp = ipv4Address;//由于packet打了恶意类型Tag，故筛选出的地址一定为恶意节点IP
                Ipv4Address m_nextHop; // Declare the next hop address to use
                m_finalRoute.clear();
                //m_finalRoute.push_back(source);
                Ipv4Address replyDst;
                for (std::vector<Ipv4Address>::iterator itr = nodeList.begin(); itr != nodeList.end();
                     itr++) {
                    m_finalRoute.push_back(*itr);
                }
                replyDst = m_finalRoute.back();

                m_finalRoute.push_back(maliciousIp);
                m_finalRoute.push_back(targetAddress);

                DsrOptionRrepHeader m_rrep;
                m_rrep.SetNodesAddress(m_finalRoute);
                DsrRoutingHeader m_dsrRoutingHeader;
                m_nextHop = maliciousIp;
                ...
                m_dsrRoutingHeader.SetDestId(GetIDfromIP(replyDst));
                // Set the route for route reply
                SetRoute(replyDst, m_nextHop);

                uint8_t length = m_rrep.GetLength();  // Get the length of the rrep header excluding the type header
                m_dsrRoutingHeader.SetPayloadLength(length + 2);
                m_dsrRoutingHeader.AddDsrOption(m_rrep);
                Ptr <Packet> m_newPacket = Create<Packet>();
                m_newPacket->SetMaliciousType(dsrP->GetMaliciousType());
                m_newPacket->AddHeader(m_dsrRoutingHeader);
                dsr->ScheduleInitialReply(m_newPacket, maliciousIp, replyDst, m_ipv4Route);
                return rreq.GetSerializedSize();
            }
            //************************************************************** Malicious Send RREP ODIE
            ...
            if (dupRequest) {
                ...
                if (targetAddress == ipv4Address)//****************************************找到了可以发RREP
                {
                    ...
                    DsrOptionRrepHeader rrep;    //oddRREP RREP RREP
                    ...
                    isPromisc = false;
                    bool a = isRouteInCache && !areThereDuplicates;  //路由抢救机制，请参照原版oddddd
                    if (a == 0)    //路由抢救机制，请参照原版oddddd
                    {            //路由抢救机制，请参照原版oddddd
                        ;        //路由抢救机制，请参照原版oddddd
                    }            //路由抢救机制，请参照原版oddddd
                    return rreq.GetSerializedSize();
                     ...
                     */
                else if (/*isRouteInCache && !areThereDuplicates*/0)  //路由抢救机制oddd
                {
                    ...
                    if (ReverseRoutes(reverseRoute)) {
                        ...
                        DsrRouteCacheEntry toSource(/*IP_VECTOR=*/ saveRoute, /*dst=*/ dst, /*expire time=*/
                                                                   ActiveRouteTimeout);
                        ...
                        if (addRoute) {
                            NS_LOG_LOGIC("We have added the route and search send buffer for packet with destination "
                                                 << dst);
                            /*
                    ...
                    dsrRoutingHeader.SetDestId(realSource.GetSubnetDirectedBroadcast(Ipv4Mask("255.255.255.0")));
                    ...
                    mainVector.push_back(ipv4Address); //转发request把当前节点压入list中
                    ...
                    */
                    ...
                    //ODDDDDDDDDDDD We make this TTL always true
                    if (1) {
                        ...
                        dsr->ScheduleInterRequest(interP, ipv4Address);
                        isPromisc = false;
                    }
                    return rreq.GetSerializedSize();
                }
        ...
        uint8_t
        DsrOptionRrep::Process(Ptr <Packet> packet, Ptr <Packet> dsrP, Ipv4Address ipv4Address, Ipv4Address source,
                               Ipv4Header const &ipv4Header, uint8_t protocol, bool &isPromisc,
                               Ipv4Address promiscSource) {
            ...
            if (0) {
                static int nn = 0;
                auto itr = find(nodeList.begin(), nodeList.end(), Ipv4Address("198.3.1.5"));
                if (itr != nodeList.end()) {
                    std::cout << "Defense mechanism cnt = " << ++nn << " ** " << Simulator::Now() << std::endl;
                    return 0;
                }
            }
            ...
            if (targetAddress == ipv4Address) {
                 ...
                 */
                DsrRouteCacheEntry toDestination(/*IP_VECTOR=*/ nodeList, /*dst=*/ dst, /*expire time=*/
                                                                ActiveRouteTimeout);
                ...
                if (addRoute) {
                    ...
                    Ipv4Address nextHop;
                    nextHop = SearchNextHop(ipv4Address, nodeList); // Get the next hop address
                ...
                uint8_t length =
                        rrep.GetLength() - 2; // The get length - 2 is to get aligned for the malformed header check
                ...
                if (cutRoute.size() >= 2) {
                    ...
                    DsrRouteCacheEntry toDestination(/*IP_VECTOR=*/ cutRoute, /*dst=*/ dst, /*expire time=*/
                                                                    ActiveRouteTimeout);
                ...
                ///////todo odie233 **** NS_ASSERT (routeCopy.back () == source);
        ...
        uint8_t
        DsrOptionSR::Process(Ptr <Packet> packet, Ptr <Packet> dsrP, Ipv4Address ipv4Address, Ipv4Address source,
                             Ipv4Header const &ipv4Header, uint8_t protocol, bool &isPromisc,
                             Ipv4Address promiscSource) {
            NS_LOG_FUNCTION(
                    this << packet << dsrP << ipv4Address << source << ipv4Address << ipv4Header << (uint32_t) protocol
                         << isPromisc);
            Ptr <Packet> p = packet->Copy();
            // Get the number of routers' address field
            uint8_t buf[2];
            ...
            if (isPromisc) {
                ...
                if (destAddress != destination) {
                    ...
                    for (int32_t i = 0; i < nNodes; ++i) {
                        ...
                        findPassive = dsrNode->PassiveEntryCheck(packet, source, destination, segsLeft, fragmentOffset,
                    ...
                    if (findPassive) {
                        ...
                        dsr->PassiveEntryCheck(packet, source, destination, segsLeft, fragmentOffset, identification,
                                               true);
                    }
                }
                ...
                nextAddress = ipv4Header.GetDestination();
//
//	  Ptr<Ipv4L3Protocol> tempIpv4L3;
//	  Ipv4Address tempAddr;
//	  Ipv4Mask tempMask;
//	  tempMask.Set(0xffffff00);
//	  std::map<Ipv4Address, Ptr<Ipv4L3Protocol>>::const_iterator iterOdie = nodeInterface.find(nextAddress);
...
//	 				  if(tempMask.IsMatch(odAddr, ipv4Address))
//	 				  {
//	 					 nextAddress = odAddr;
//	 					  break;
//	 				  }
//	 	  }
                ...
                NS_LOG_DEBUG("The next address of source route option " << nextAddress << " and the nextAddressIndex: "
                ...
                Ipv4Header vvv = ipv4Header;
                dsr->ForwardPacket(dsrP, newSourceRoute, vvv, realSource, nextAddress, targetAddress, protocol,
                                   m_ipv4Route);
            }
            return sourceRoute.GetSerializedSize();
        }
        ...
        uint8_t
        DsrOptionRerr::Process(Ptr <Packet> packet, Ptr <Packet> dsrP, Ipv4Address ipv4Address, Ipv4Address source,
                               Ipv4Header const &ipv4Header, uint8_t protocol, bool &isPromisc,
                               Ipv4Address promiscSource) {
            return 0;
            NS_LOG_FUNCTION(
...
//      uint32_t serialized = DoSendError (p, rerrUnsupport, rerrSize, ipv4Address, protocol);
        ...
        uint8_t DsrOptionRerr::DoSendError(Ptr <Packet> p, DsrOptionRerrUnreachHeader &rerr, uint32_t rerrSize,
        ...
        uint8_t
        DsrOptionAckReq::Process(Ptr <Packet> packet, Ptr <Packet> dsrP, Ipv4Address ipv4Address, Ipv4Address source,
                                 Ipv4Header const &ipv4Header, uint8_t protocol, bool &isPromisc,
                                 Ipv4Address promiscSource) {
            NS_LOG_FUNCTION(
                    this << packet << dsrP << ipv4Address << source << ipv4Header << (uint32_t) protocol << isPromisc);
        ...
        uint8_t
        DsrOptionAck::Process(Ptr <Packet> packet, Ptr <Packet> dsrP, Ipv4Address ipv4Address, Ipv4Address source,
                              Ipv4Header const &ipv4Header, uint8_t protocol, bool &isPromisc,
                              Ipv4Address promiscSource) {
            NS_LOG_FUNCTION(
                    this << packet << dsrP << ipv4Address << source << ipv4Header << (uint32_t) protocol << isPromisc);
