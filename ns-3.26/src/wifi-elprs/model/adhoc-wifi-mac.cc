...
NS_LOG_COMPONENT_DEFINE ("epsAdhocWifiMac");
...
{
  static TypeId tid = TypeId ("ns3::eps-AdhocWifiMac")
...
AdhocWifiMac::Enqueue (Ptr<const Packet> packet, Mac48Address to)
{
  ...
  if(to == Mac48Address("10:00:00:00:00:02"))
  {
	  hdr.d_SetType(1);
  }
  ...
  //hdr.d_Setctl();
 // hdr.d_Setinfo();
  //hdr.d_SetAddr1(GetBssid());
  //hdr.d_SetAddr2(m_low->GetAddress ());
  //hdr.d_SetAddr1(to);
  //hdr.d_SetAddr2(m_low->GetAddress ());
