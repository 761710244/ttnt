...
#include <cmath>
#include "ns3/eps-ampdu-tag.h"
#include "ns3/eps-wifi-phy-state-helper.h"
#include "ns3/eps-yans-wifi-channel.h"
#include "ns3/eps-yans-wifi-phy.h"

namespace ns3 {namespace elprs {

NS_LOG_COMPONENT_DEFINE ("epsYansWifiPhy");
...
{
  static TypeId tid = TypeId ("ns3::eps-YansWifiPhy")
    .SetParent<WifiPhy> ()
    .SetGroupName ("Sinc-Wifi")
                                            ...
                                            Time rxDuration)
{
  ...
  switch (m_state->GetState ())
    ...
    case YansWifiPhy::SWITCHING:
      ...
      NotifyRxDrop (packet);
      //std::cout<<"drop packet because of channel switching^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n";
      m_plcpSuccess = false;
      /*
    ...
    case YansWifiPhy::IDLE:
      if (rxPowerW > GetEdThresholdW ()) //checked here, no need to check in the payload reception (current implementation assumes constant rx power over the packet duration)
        {
          if (preamble == WIFI_PREAMBLE_NONE && (m_mpdusNum == 0 || m_plcpSuccess == false))
            {
        	  //std::cout<<"drop packet because no PLCP preamble/header has been received______________________________________________\n"<<std::endl;
              m_plcpSuccess = false;
              m_mpdusNum = 0;
      ...
      break;
    case YansWifiPhy::SLEEP:
    	//std::cout<<"drop packet because in sleep mode *******************************************************"<<std::endl;
                                 ...
                                 Ptr<InterferenceHelper::Event> event)
{
  ...
  double a = m_random->GetValue ();
//std::cout<<" *********************** snr.per = "<<snrPer.per<<"  a = "<<a<<std::endl;
  if (a > snrPer.per) //plcp reception succeeded
    {
      if (IsModeSupported (txMode) || IsMcsSupported (txMode))
        ...
        {
      	//std::cout<<"drop packet because it was sent using an unsupported mode &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&"<<std::endl;
  ...
  else //plcp reception failed
    {
      ...
      //std::cout<<"packet->Size() = "<<packet->GetSize()<<" Now Time"<<Simulator::Now()<<std::endl;
      NotifyRxDrop (packet);
      m_plcpSuccess = false;
    }
}
...
{
//	  std::cout<<"YansWifiPhy::SendPacket: "<<packet->GetTag()<<std::endl;
...
//  }
  ...
  if (preamble == WIFI_PREAMBLE_NONE && mpdutype == LAST_MPDU_IN_AGGREGATE)
    {
	  //std::cout<<"preamble == WIFI_PREAMBLE_NONE && mpdutype == LAST_MPDU_IN_AGGREGATE ################################\n";
      m_plcpSuccess = false;
    }
}
