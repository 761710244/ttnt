...
NS_LOG_COMPONENT_DEFINE ("epsApWifiMac");
...
{
  static TypeId tid = TypeId ("ns3::eps-ApWifiMac")
...
{
//	static uint16_t k = 0;
	static uint16_t ll = 0;
	//std::cout<<"XXXXXXXXXXXXXXXWWWWWWWWWWWWWWWWWWWWW"<<std::endl;;
	NS_LOG_FUNCTION(this << packet << hdr);
	Mac48Address from = hdr->GetAddr2();
	//Whether needs XNP function
	if (1) {
	Mac48Address odmac;
		odmac = "10:00:00:00:00:02";
		if (hdr->IsData()) {
			if (!hdr->IsFromDs() && !hdr->IsToDs()) {
				Ptr<Packet> copy2 = packet->Copy();
				elprs::Ipv4Header iheader;
				LlcSnapHeader llc;
				uint32_t sizellc = copy2->RemoveHeader(llc);
				if (sizellc == 0) {
					std::cout << "can not remove LLCheader" << std::endl;
				} else {
					uint32_t size = copy2->RemoveHeader(iheader);
					if (size == 0) {
						std::cout << "can not remove ipv4header" << std::endl;
					} else if (hdr->GetAddr1() == odmac) //---------------------------------FIX if addr
							{
						//    		  std::cout<<"iheader.Get_i_Field_1(): "<<std::hex<<uint16_t(iheader.Get_i_Field_1())<<std::endl;
						if (iheader.Get_i_Field_1() == 0x61) //---------------------------------FIX
								{
//							if(hdr->GetAddr2()==Mac48Address("00:00:00:00:00:01"))
//							{
//								std::cout<<" XNP REP :01::::: "<<++k<<std::endl;
//							}
			//				std::cout<<"XNPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP\n";
//        			  std::cout<<"find a XNP request"<<std::endl;
							...
							XnpRepHeader xnprep;
							xnprep.x_SetVersionNum(0x80);
							xnprep.x_SetId(0xa8);
							reply->AddHeader(xnprep);

							elprs::Ipv4Header irplyHder;
							irplyHder.Set_i_Field_1(0x61);
							irplyHder.Set_i_HeaderLength();
							irplyHder.Set_i_ServeType();
							irplyHder.Set_i_InfoRecognize();
							irplyHder.Set_i_FrameNum();
							irplyHder.Set_i_frameCounts();
							irplyHder.Set_i_MaxHoop();
							irplyHder.SetPayloadSize(0);
							Ipv4Address xnp_rep_source;
							xnp_rep_source.Set(0xa000002);
							irplyHder.SetSource(xnp_rep_source);
							irplyHder.SetDestination(iheader.GetSource());
							reply->AddHeader(irplyHder);

							WifiMacHeader rhdr;
							rhdr.SetAddr1(from);
							rhdr.SetAddr2(GetAddress());
							rhdr.SetAddr3(odmac);
							rhdr.SetTypeData();

							LlcSnapHeader llc;
							llc.SetType(0x0800); //---------------------------------FIX
							reply->AddHeader(llc);

							//reply->AddHeader(rhdr);
							if(irplyHder.GetSource()==Ipv4Address("10.0.0.2"))
							{
								std::cout<<"llllllllllllll------ "<<++ll<<
										" PacS = "<<reply->GetSize()<<" dst: "<<irplyHder.GetDestination()
										<<" GetNow = "<<Simulator::Now()<<std::endl;
								std::cout<<std::endl;
							}

							m_dca->Queue(reply, rhdr);
							//    			  std::cout<<"apwifimac::receive  (): "<<std::hex<<uint16_t(irplyHder.Get_i_Field_1())<<std::endl;
						}
					}
				}  //sizellc==0 else
			}  // if (!hdr->IsFromDs ()
