...
NS_LOG_COMPONENT_DEFINE ("epsMacLow");
 ...
 */
class PhyMacLowListener : public ns3::elprs::WifiPhyListener
{
public:
  /**
   ...
   * \param macLow
   */
  PhyMacLowListener (ns3::elprs::MacLow *macLow)
    : m_macLow (macLow)
  {
  }
...
{
  static TypeId tid = TypeId ("ns3::eps-MacLow")
    .SetParent<Object> ()
    .SetGroupName ("Wifi")
...
//  std::cout<<"MACLOW::REVOK: "<<packet->GetTag()<<"Simulator::Now = "<<Simulator::Now()<<std::endl;
  WifiMacHeader hdr;
  packet->RemoveHeader (hdr);
  m_lastReceivedHdr = hdr;

  if(hdr.GetAddr1()==Mac48Address("00:00:00:00:00:05")&&packet->GetSize()<50)
  {
	  std::cout<<std::endl;
  }
...
MacLow::StartDataTxTimers (WifiTxVector dataTxVector)
{
  ...
  if (m_txParams.MustWaitNormalAck ())
    {
	  //**********************************************************OD
      Time timerDelay = txDuration + MicroSeconds(470)/*GetAckTimeout ()*/;
      //**********************************************************OD
