...
Ipv4Interface::Send (Ptr<Packet> p, const Ipv4Header & hdr, Ipv4Address dest)
{
	  ...
	  Ipv4Address sincgarsNodesIP;
	  Ipv4Mask stdMask;
	  stdMask.Set(0xffff0000);
	  sincgarsNodesIP.Set(0xc6030000);///////////////////////
	  if (!IsUp ())
	    {
	      return;
	    }
...
if(0)
{
				  if( ((!m_device->getSwitch()) && (stdMask.IsMatch(sincgarsNodesIP, hdr.GetSource()))
						  && m_device->GetXnpTimeState()==0 && hdr.GetSource() == Ipv4Address("198.3.1.3")
						)  || ((!m_device->getSwitch()) && (stdMask.IsMatch(sincgarsNodesIP, hdr.GetSource()))
								  && m_device->GetXnpTimeState()==0 && Simulator::Now()>=Seconds(19.8)))// if(!m_device->getSwitch()&&IP地址是sincgars节点)
				  {
					  Ptr<ArpL3Protocol> arp = m_node->GetObject<ArpL3Protocol> ();
		  ...
		  if ( (found /*&& m_device->getSwitch() && stdMask.IsMatch(sincgarsNodesIP, hdr.GetSource())
				  ) || (found && !stdMask.IsMatch(sincgarsNodesIP, hdr.GetSource())*/) ) //if ((found && m_device->getSwitch()&&IP地址是sincgars节点) || (节点的IP地址不是SINCGARS&&found) )
		  {
