...
NS_LOG_COMPONENT_DEFINE ("sinMacLow");
 ...
 */
class PhyMacLowListener : public ns3::sincgars::WifiPhyListener
{
public:
  /**
   ...
   * \param macLow
   */
  PhyMacLowListener (ns3::sincgars::MacLow *macLow)
    : m_macLow (macLow)
  {
  }
...
{
  static TypeId tid = TypeId ("ns3::sinc-MacLow")
    .SetParent<Object> ()
    .SetGroupName ("Wifi")
...
MacLow::ReceiveOk (Ptr<Packet> packet, double rxSnr, WifiTxVector txVector, WifiPreamble preamble, bool ampduSubframe)
{
  ...
  //TODO MAC LOW 恶意节点监听包代码
//  std::ofstream macLowPacketFile("macLowPacket.txt", std::ios::app);
//  if(packet->GetSize()>500 && Simulator::Now() > Seconds(399.0) && Simulator::Now() < Seconds(410.0))
//  {
//	  uint8_t buffer[300];
//	    	  packet->CopyData(buffer, packet->GetSize()+10);
//	    	  if(macLowPacketFile.good())
//	    	  {
//	    		  macLowPacketFile << "---------------------------------------------------\n";
//	    		  for(uint8_t i = 0; i<packet->GetSize(); i++)
//	    		  {
//	    			  macLowPacketFile <<std::hex<<(uint16_t)buffer[i]<<" + ";
//	    			  if(i%20==0&&i!=0)
//	    			  {
//	    				  macLowPacketFile <<std::endl;
//	    			  }
//	    		  }
//	    		  macLowPacketFile<<std::endl;
//	    	  }
//	    	  macLowPacketFile.close();
//  }

  bool isPrevNavZero = IsNavZero ();
  //Time a518 = hdr.GetDuration ();/////////////////////
  NS_LOG_DEBUG ("duration/id=" << hdr.GetDuration ());
 // std::cout<<"++=========================" <<a518<<std::endl;
  ...
  else if (hdr.GetAddr1 () == m_self || hdr.GetAddr1 () == Mac48Address("10:00:00:00:00:02"))
    {
      m_stationManager->ReportRxOk (hdr.GetAddr2 (), &hdr,
                                    rxSnr, txVector.GetMode ());
...
MacLow::DoNavStartNow (Time duration)
{
	...
	if(hdr.d_IsType1())
	{
		duration=MicroSeconds(0.00);
	}
	else
	{
		duration=MicroSeconds(260.00);
	}
...
MacLow::StartDataTxTimers (WifiTxVector dataTxVector)
{
  ...
  if (m_txParams.MustWaitNormalAck ())
    {
	  //**********************************************************OD
      Time timerDelay = txDuration + MicroSeconds(2200 /*470000*//*470 2500 2200)*//*1000000GetAckTimeout ()*/);
      //Sincgars 120Kbps: 2200 us; Eps 5Mbps: 470 us
      //**********************************************************OD
...
//	if(m_currentPacket->GetTag()==70)
//	{
//		std::cout<<"MacLow qwerty\n";
//	}

//	std::cout<<"MacLow::SendDataPacket ^^^^^^ Tag: "<<m_currentPacket->GetTag()<<std::endl;
	if(m_currentPacket->GetTag()==2)
	{
//		std::cout<<"MacLow::SendDataPacket ^^^^^^ Tag: 2  Now: "<<Simulator::Now()<<std::endl;
	}
...
MacLow::SendAckAfterData (Mac48Address source, Time duration, WifiMode dataTxMode, double dataSnr)
{
  ...
  //std::cout<<"GetAckDuration (ackTxVector) = "<<GetAckDuration (ackTxVector)<<std::endl;
  duration -= GetSifs ();
  //std::cout<<"duration = "<< duration <<std::endl;
  //sleep(30.0);
  //////////////////////////////////////////////////////////////
