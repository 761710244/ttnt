...
#include "ns3/sinc-wifi-mac-queue.h"
...
{
  static TypeId tid = TypeId ("ns3::sinc-WifiMacQueue")
    .SetParent<Object> ()
    .SetGroupName ("Wifi")
...
WifiMacQueue::Enqueue (Ptr<const Packet> packet, const WifiMacHeader &hdr)
{
  ...
  m_queue.push_back (Item (packet, hdr, now)); //这里的Packet最前面是LLC HEADER
  m_size++;
}
...
//  	std::cout<<"AdhocWifiMac::Dequeue pppppppp"<<i.packet->GetTag() <<
