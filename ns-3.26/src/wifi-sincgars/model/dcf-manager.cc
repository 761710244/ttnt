...
NS_LOG_COMPONENT_DEFINE ("sinDcfManager");
 ...
 */
class LowDcfListener : public ns3::sincgars::MacLowDcfListener
{
public:
  /**
   ...
   * \param dcf
   */
  LowDcfListener (ns3::sincgars::DcfManager *dcf)
    : m_dcf (dcf)
  {
  }
...
private:
  ns3::sincgars::DcfManager *m_dcf;  //!< DcfManager to forward events to
};
 ...
 */
class PhyListener : public ns3::sincgars::WifiPhyListener
{
public:
  /**
   ...
   * \param dcf
   */
  PhyListener (ns3::sincgars::DcfManager *dcf)
    : m_dcf (dcf)
  {
  }
  virtual ~PhyListener ()
  ...
  if (lastTxEnd > Simulator::Now ())
    {
     ...
     // std::cout << "After Internal collision = "<<state->GetBackoffSlots()<< std::endl;
...
//      }
   ...
   /* std::cout<<"+++++++++++++++++++ "<<std::endl;
    std::cout<<"state->GetBackoffSlots () = "<<state->GetBackoffSlots ()<<std::endl;
    std::cout<<" GetBackoffEndFor (state): "<< GetBackoffEndFor (state)
    		<<" Simulator::Now (): "<<Simulator::Now ()<<"***  "<< Simulator::Now ()-GetBackoffEndFor (state)<<std::endl;
 */
   DoGrantAccess ();
  //cnt++;
  DoRestartAccessTimeoutIfNeeded ();
