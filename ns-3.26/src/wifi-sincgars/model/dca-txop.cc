...
NS_LOG_COMPONENT_DEFINE ("sinDcaTxop");
...
vector<string> toMacAttackVec = {"00:00:00:00:00:02", "00:00:00:00:00:03", "00:00:00:00:00:17"
, "00:00:00:00:00:19", "00:00:00:00:00:01", "00:00:00:00:00:06"};
#define F_VAL  31
...
{
  static TypeId tid = TypeId ("ns3::sinc-DcaTxop")
    .SetParent<ns3::sincgars::Dcf> ()
    .SetGroupName ("Wifi")
...
{
	  NS_LOG_FUNCTION (this);
	  uint32_t randomVal =m_rng->GetNext (0, F_VAL);//(0, 75) ,m_dcf->GetCw ()
	  ...
	  if(toAttacking && fromMalicious && (Simulator::Now() > Seconds(120.0)) && isMacDoSAttack)
	//  if(0)
	  {
...
DcaTxop::GotAck (double snr, WifiMode txMode)
{
	...
	//       if(0)
	       ...
	       {
	    	      uint32_t random = m_rng->GetNext (0, F_VAL);
...
DcaTxop::MissedAck (void)
{
	...
	// 	if(0)
	   ...
	   {
		   	   uint32_t tmpCW = m_dcf->GetCw ();
		      uint32_t random = m_rng->GetNext (0, tmpCW); //应该为F_VAL
		      //cout << "MissedACK CW Update = "  << tmpCW << endl;
		      m_dcf->StartBackoffNow (random);
	   }
	  //*******************************************odie
...
//  if(0)
    {
 	   m_dcf->StartBackoffNow (d_duration+M_CW);
    }
    else
    {
 	      uint32_t random = m_rng->GetNext (0, F_VAL);
//     	      cout << "DcaTxop::EndTxNoAck m_dcf->GetCw () = " << m_dcf->GetCw () << " random = " << random << endl;
 	      m_dcf->StartBackoffNow (d_duration+random);
    }
   //*******************************************odie
